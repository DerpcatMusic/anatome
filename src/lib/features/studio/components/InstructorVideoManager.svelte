<script lang="ts">
  ...
  type VideoList = FunctionReturnType<typeof api.instructorVideos.listAll>;
  ...
  async function startUpload() {
    ...
    try {
      const result = await useConvexClient().action(api.videosUpload.requestUpload, {
        title: title.trim(),
      ...
      setTimeout(() => {
        if (uploadStatus === "processing") {
          ...
          draftVideoId = null;
          tab = "library";
          // Query auto-updates via Convex subscription
        }
      }, 3000);
    } catch (reason) {
      uploadStatus = "error";
  ...
  async function deleteVideo(videoId: Id<"videos">) {
    ...
    actionError = "";
    try {
      await useConvexClient().mutation(api.instructorVideos.deleteVideo, { videoId });
      // Query auto-updates via Convex subscription
    } catch (reason) {
  ...
  async function publishVideo(videoId: Id<"videos">) {
    ...
    actionError = "";
    try {
      await useConvexClient().mutation(api.instructorVideos.updateMetadata, { videoId, status: "published" });
      // Query auto-updates via Convex subscription
    } catch (reason) {
  ...
  async function saveEdit(videoId: Id<"videos">) {
    actionId = videoId;
    try {
      await useConvexClient().mutation(api.instructorVideos.updateMetadata, {
        videoId,
        title: editTitle,
