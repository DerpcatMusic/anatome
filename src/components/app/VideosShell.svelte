<script lang="ts">
  ...
  import { authQuery, initAuth, getCachedRole } from "../../lib/auth/session.svelte";
  import { convex } from "../../lib/convex/client";
  import { equipmentLabel, durationLabel } from "../../lib/labels";
  import AppLayout from "./AppLayout.svelte";
  import PageShell from "./PageShell.svelte";
  import Notice from "../ui/Notice.svelte";
  import InstructorVideoManager from "./InstructorVideoManager.svelte";
...
{:else if dataResource.error}
  <AppLayout>
    <div class="state-card">
      ...
      <button type="button" class="btn btn--ghost" onclick={() => dataResource.refetch()}>לנסות שוב</button>
    </div>
  </AppLayout>
{:else if data}
  <AppLayout>
    ...
    >
      ...
      {#if data.videos.length === 0}
        <div class="state-card">
          <p>אין עדיין וידאו פעיל לשבוע הזה. בדוק מאוחר יותר.</p>
        </div>
      {:else}
        ...
        {#each data.videos as item}
          <article class:locked={item.locked} class:selected={item.selected} class="video-card">
            ...
            <div class="video-card__body">
              ...
              <div class="actions">
                {#if item.selected}
                  <a href={`/watch?videoId=${item.video._id}`}>לצפות</a>
                {:else}
                  <button
                    type="button"
...
<style>
  ...
  .video-card__top .status {
    margin: 0;
    color: var(--muted);
  }

  .video-card__top .duration {
    color: var(--muted);
  }
