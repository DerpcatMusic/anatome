...
function WatchShell($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		const playbackResource = resource(() => auth.isAuthenticated && videoId(), async (shouldFetch) => {
			...
			return await authQuery(api.video.playback.getViewerPlayback, { videoId: videoId() });
		});
		const playbackData = derived(() => playbackResource.current);
		const client = useConvexClient();
		const tokenResource = resource(() => auth.isAuthenticated && playbackData()?.video.playbackId ? playbackData().video._id : null, async (resolvedVideoId) => {
		...
		async function updateProgress(progress) {
			if (!videoId()) return;
			await client.mutation(api.video.playback.updateProgress, {
				videoId: videoId(),
				...progress
			});
		}
		if (auth.isLoading) {
			...
			$$renderer.push(`<!----> <a class="button-link svelte-ndauuo" href="/u/videos">חזרה לספרייה</a></div>`);
			...
			if (playbackData().video.playbackId && playbackToken()) {
				...
				Notice($$renderer, {
					tone: "danger",
					children: ($$renderer) => {
						$$renderer.push(`<!---->${escape_html(tokenResource.error?.message ?? "שגיאה באישור הצפייה")}`);
					},
					$$slots: { default: true }
				});
				$$renderer.push(`<!----></div>`);
				...
				Notice($$renderer, {
					children: ($$renderer) => {
						$$renderer.push(`<!---->מאשרים צפייה מאובטחת...`);
					},
					$$slots: { default: true }
				});
				$$renderer.push(`<!----></div>`);
